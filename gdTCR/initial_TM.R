...
ppt <- theme(text=element_text(size=30), axis.text=element_text(size = 30), axis.title=element_text(size = 30), plot.title=element_text(size = 30), 
             strip.text=element_text(size=30), legend.key.size=unit(3,"cm"))
...
ggsave(paste(plot_folder,'Vg usage in IEL_PBL.pdf'),width=14,height=9.33)
...
ggsave(paste(plot_folder,'Control Vg usage in PBL vs. IEL.pdf'),width=10,height=7)
...
ggsave(paste(plot_folder,'Control PBL vs. IEL Gamma vs. Delta CDR3 freq bar plots.pdf'),width=10,height=10)
...
g = ggplot(clone_summary_Active_GFD_PBL,aes(factor(ID),Freq,fill=Clone))
g = g + geom_bar(stat='identity')
